...
SELECT DNO, SALARY FROM EMPLOYEE
```
**Obervação:** Nesse QUERY observamos que não ha nenhuma ordenação ou especificação de unicidade para elementos.

```{sql connection=con}
SELECT DISTINCT DNO, SALARY FROM EMPLOYEE
```
**Obervação:** Já nesse QUERY inserimos o `DISTINCT` para evitar repetições, mas não existe nenhum comando ordenando os dados.

```{sql connection=con}
SELECT DISTINCT DNO, SALARY FROM EMPLOYEE 
ORDER BY DNO, SALARY
```
**Obervação:** Por fim, esse QUERY observamos o `DISTINCT` aplicado no chunk anterior e agora o `ORDER BY` ordenando os dados primeiro pela `DNO` e depois pela coluna `SALARY`
...
**Resposta:** Sim, ambas vão necessariamente selecionar o `STATE = 'MA'`, ele podendo ser com `ACRES > 1000` ou com acres < 100, mas com `FEE = 0.0`
...
**Resposta:** Sim, pela propriedade distributiva do operador OR sobre o AND.
...
**Resposta:** Não, A expressão 1 e a expressão 3 são equivalentes entre si, mas a expressão 2 difere das outras duas. A segunda exige que ambas as negações ocorram simultaneamente.
...
**Resposta:** Somente a primeira e a terceira são equivalentes, a segunda difere delas pelo fato de não exigir que as negações ocorram simultaneamente. 
